...
**1 commit nuevo** — brianmojena. Drill-down desde las tarjetas de métricas de usuarios: nueva query `admin_segment_users` que devuelve la lista paginada de usuarios detrás de cada segmento de `admin_user_metrics`.

---

### Área 1: Users — `admin_segment_users` con búsqueda y paginación (brianmojena, 20:48)

- **`feat(users): drill down from each metrics card into its users`** — Complementa `admin_user_metrics` (Ago 19): cada tarjeta de métrica ahora puede abrirse para ver los usuarios que la componen.

  1. **Nueva query GraphQL `admin_segment_users`** — Lista paginada de usuarios de cualquier segmento de `admin_user_metrics`, con búsqueda sobre nombre/email/teléfono. Los segmentos se derivan de la misma llamada `get_metrics_sources` vía `build_segment_spec`, garantizando que el número en la tarjeta y la lista siempre coincidan. Hay un test que aserta exactamente esa invariante.

  2. **`AdminUserRowType` construido campo a campo** — En lugar de `**to_strawberry_dict(user)`, cada campo se extrae explícitamente. Previene que el password filtre si se añade un campo nuevo a `User`, y evita el patrón "unexpected keyword argument" documentado en CLAUDE.md.

  3. **Campos `isCourier/isBusiness/isActive` en cada fila** — Derivados de los mismos joins que usan las métricas, permitiendo al UI mostrar a qué apps pertenece cada usuario.

  4. **`limit` clampeado a 200, `offset` >= 0** — Validación server-side.

---

### Puede dar bateo

1. **`limit` máximo de 200 — segmentos grandes truncados sin aviso**: Si un segmento tiene >200 usuarios, el drill-down los trunca silenciosamente. Si el UI no muestra el total real ni indica que hay más resultados, el usuario creerá ver todos. La query necesita `total_count` + `has_more` o paginación real.

2. **Búsqueda sin índice confirmado — posible collection scan**: La búsqueda sobre nombre/email/teléfono puede ser costosa si esos campos no tienen índices en MongoDB. Con solo los índices nuevos de `createdAt`/`lastSeenAt` (Ago 19) y el índice text previo, una búsqueda por nombre exacto sobre todos los usuarios es potencialmente lenta.

3. **Race condition entre conteo de tarjeta y lista**: Aunque usan el mismo `build_segment_spec`, las queries se ejecutan en momentos distintos. Un usuario que se une entre la carga de métricas y la apertura del drill-down puede aparecer en la lista pero no en el conteo de la tarjeta (o viceversa).

4. **`admin_segment_users` — confirmar guard de autorización**: Los commit messages no mencionan explícitamente `require_role("admin")` u otro decorator. Si la query no está protegida, cualquier usuario autenticado podría listar usuarios de la plataforma con nombre/email/teléfono.

5. **`isCourier/isBusiness/isActive` con joins por fila — potencial N+1**: Si estos campos se resuelven con joins separados para cada usuario de la lista (en lugar de batch desde `get_metrics_sources`), la performance puede degradarse significativamente con listas cercanas al límite de 200.

6. **Test de invariante solo en tiempo de test**: La garantía de que "la lista nunca desacuerda con la tarjeta" solo se verifica en tests, no en runtime. Una race condition real entre las dos queries no haría fallar ningún test.
