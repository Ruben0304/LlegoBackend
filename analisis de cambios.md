...
4. **Tokens desactivados permanentemente por `UNREGISTERED`/`INVALID_ARGUMENT`**: Si FCM devuelve estos códigos por error transitorio (raro pero posible), el token queda desactivado y el dispositivo deja de recibir notificaciones hasta que el usuario re-registre. Confirmar que la lógica de desactivación es correcta y que hay mecanismo de re-registro en la app Android.

5. **Notificador KYC usa siempre `platform IOS`**: El commit lo menciona como justificación para no afectar callers actuales. Si en algún momento se necesita notificación KYC a Android, requiere trabajo adicional.
