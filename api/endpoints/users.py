...
from utils.s3 import upload_file, generate_presigned_url, delete_file
from utils.rate_limit import limiter, RATE_LIMIT_UPLOADS
from services.image_processing import process_image_for_store
...
async def update_user_avatar(
    ...
    # Upload to S3
    try:
        image_path = await upload_file(processed_content, "users/avatars", user_id, extension)
    except Exception as e:
    ...
    return {
        ...
        "avatar_url": generate_presigned_url(updated_user.avatar) if updated_user.avatar else None
    }
