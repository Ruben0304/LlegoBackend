...
class AppConfigMutations:
    ...
    @strawberry.mutation(description="Actualizar configuración de la aplicación de negocios (requiere rol admin)")
    async def update_business_app_config(
        self,
        info: Info,
        input: UpdateAppConfigInput,
        jwt: str
    ) -> Optional[BusinessAppConfigType]:
        """
        Update business app configuration. Requires admin role.
        Only updates fields that are provided (partial update).
        Updates MongoDB and refreshes Redis cache.
        """
        # Require admin role
        require_role(jwt, info, ["admin"])

        # Get current config
        current_config = await business_app_config_repo.get()
        if not current_config:
            raise Exception("Configuración de la app de negocios no encontrada")
        ...
        # Update config in database (also refreshes Redis cache)
        updated_config = await business_app_config_repo.update(current_config.id, updates)
        if not updated_config:
            raise Exception("Error al actualizar la configuración")
